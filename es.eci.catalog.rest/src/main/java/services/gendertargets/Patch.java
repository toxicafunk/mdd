...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		GenderTarget entity = (GenderTarget) message.body();
		
		GenderTargetBean jooqBean = POJOBeanConverter.entity2Bean(create, entity);
		
		GenderTargetsRecord entityRecord = create.fetchOne(GENDER_TARGETS, GENDER_TARGETS._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			GenderTargetBean updatedEntity = new GenderTargetBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toGenderTarget(create, updatedEntity), options);
		} else {
