...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<GenderTargetBean> entities = query.fetchInto(GenderTargetBean.class);
		int total = create.selectCount().from(GENDER_TARGETS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			options.setCodecName("gendertargetsinversecodec");
			headers.add("type", "object");
			message.reply(BeanPOJOConverter.toGenderTarget(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<GenderTarget> entityList = entities.stream().map(e -> BeanPOJOConverter.toGenderTarget(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d GenderTargets", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = GENDER_TARGETS.field(split[0]);
			Field<?> field1 = GENDER_TARGETS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(GENDER_TARGETS._ID);
			fields.add(GENDER_TARGETS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = GENDER_TARGETS._ID.equal(Integer.valueOf(idParameter));
		}
