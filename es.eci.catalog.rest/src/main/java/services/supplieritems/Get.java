...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<SupplierItemBean> entities = query.fetchInto(SupplierItemBean.class);
		int total = create.selectCount().from(SUPPLIER_ITEMS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toSupplierItem(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<SupplierItem> entityList = entities.stream().map(e -> BeanPOJOConverter.toSupplierItem(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d SupplierItems", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = SUPPLIER_ITEMS.field(split[0]);
			Field<?> field1 = SUPPLIER_ITEMS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(SUPPLIER_ITEMS._ID);
			fields.add(SUPPLIER_ITEMS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = SUPPLIER_ITEMS._ID.equal(Integer.valueOf(idParameter));
		}
