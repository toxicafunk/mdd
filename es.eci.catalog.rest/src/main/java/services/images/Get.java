...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<ImageBean> entities = query.fetchInto(ImageBean.class);
		int total = create.selectCount().from(IMAGES).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toImage(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Image> entityList = entities.stream().map(e -> BeanPOJOConverter.toImage(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Images", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = IMAGES.field(split[0]);
			Field<?> field1 = IMAGES.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(IMAGES._ID);
			fields.add(IMAGES._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = IMAGES._ID.equal(Integer.valueOf(idParameter));
		}
