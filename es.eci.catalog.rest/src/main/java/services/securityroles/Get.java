...
public class Get extends ECIServiceVerticle {
	...
	public static SecurityRole fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_ROLE_).where(SECURITY_ROLE_._ID_.eq(id)).fetchInto(SecurityRole.class).get(0);
	}

	public static List<SecurityRole> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(SECURITY_ROLE_).where(SECURITY_ROLE_._ID_.in(ids)).fetchInto(SecurityRole.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "SecurityRole_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (SecurityRole) r.into(SECURITY_ROLE_).into(SecurityRole.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d SecurityRoles", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(SECURITY_ROLE_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			logger.info("No securityroles found!");
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (SecurityRole) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (SecurityRole) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = SECURITY_ROLE_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(SECURITY_ROLE_._ID_.asc());
			fields.add(SECURITY_ROLE_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(SECURITY_ROLE_);

		if (initialize.contains("privileges")) {
		
			clause.join(SECURITY_ACCESS_PRIVILEGE_, JoinType.LEFT_OUTER_JOIN).on(SECURITY_ROLE_. _ID_.eq(SECURITY_ACCESS_PRIVILEGE_._SECURITY_ROLE_ID_));
	
		}
		if (initialize.contains("policies")) {
		
			clause.join(SECURITY_ROW_LEVEL_POLICY_, JoinType.LEFT_OUTER_JOIN).on(SECURITY_ROLE_. _ID_.eq(SECURITY_ROW_LEVEL_POLICY_._SECURITY_ROLE_ID_));
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = SECURITY_ROLE_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = SECURITY_ROLE_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.security_role_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
