...
public class Get extends ECIServiceVerticle {
	...
	public static SecurityAttribute fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_ATTRIBUTE_).where(SECURITY_ATTRIBUTE_._ID_.eq(id)).fetchInto(SecurityAttribute.class).get(0);
	}

	public static List<SecurityAttribute> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(SECURITY_ATTRIBUTE_).where(SECURITY_ATTRIBUTE_._ID_.in(ids)).fetchInto(SecurityAttribute.class);
	}

	public static List<SecurityAttribute> fecthByMasterId(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_ATTRIBUTE_).where(SECURITY_ATTRIBUTE_._SECURITY_ENTITY_ID_.eq(id)).fetchInto(SecurityAttribute.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "SecurityAttribute_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (SecurityAttribute) r.into(SECURITY_ATTRIBUTE_).into(SecurityAttribute.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d SecurityAttributes", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(SECURITY_ATTRIBUTE_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			logger.info("No securityattributes found!");
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (SecurityAttribute) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (SecurityAttribute) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = SECURITY_ATTRIBUTE_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(SECURITY_ATTRIBUTE_._ID_.asc());
			fields.add(SECURITY_ATTRIBUTE_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(SECURITY_ATTRIBUTE_);
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = SECURITY_ATTRIBUTE_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = SECURITY_ATTRIBUTE_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.security_attribute_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
