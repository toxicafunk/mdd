...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		Item entity = (Item) message.body();
		
		ItemBean jooqBean = POJOBeanConverter.entity2Bean(create, entity);
		
		ItemsRecord entityRecord = create.fetchOne(ITEMS, ITEMS._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			ItemBean updatedEntity = new ItemBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toItem(create, updatedEntity), options);
		} else {
