...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<ItemBean> entities = query.fetchInto(ItemBean.class);
		int total = create.selectCount().from(ITEMS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			options.setCodecName("itemsinversecodec");
			headers.add("type", "object");
			message.reply(BeanPOJOConverter.toItem(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Item> entityList = entities.stream().map(e -> BeanPOJOConverter.toItem(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Items", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = ITEMS.field(split[0]);
			Field<?> field1 = ITEMS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(ITEMS._ID);
			fields.add(ITEMS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = ITEMS._ID.equal(Integer.valueOf(idParameter));
		}
