...
public class Get extends ECIServiceVerticle {
	...
	public static Material fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(MATERIAL_).where(MATERIAL_._ID_.eq(id)).fetchInto(Material.class).get(0);
	}

	public static List<Material> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(MATERIAL_).where(MATERIAL_._ID_.in(ids)).fetchInto(Material.class);
	}

	public static List<Material> fecthByMasterId(DSLContext ctx, Integer id) {
		return ctx.select().from(MATERIAL_).where(MATERIAL_._COURSE_ID_.eq(id)).fetchInto(Material.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "Material_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (Material) r.into(MATERIAL_).into(Material.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d Materials", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(MATERIAL_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (Material) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (Material) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = MATERIAL_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(MATERIAL_._ID_.asc());
			fields.add(MATERIAL_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(MATERIAL_);

		if (initialize.contains("item")) {
			clause.join(ITEM_, JoinType.LEFT_OUTER_JOIN).on(MATERIAL_.ITEM_.eq(ITEM_._ID_));
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = MATERIAL_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = MATERIAL_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.material_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
