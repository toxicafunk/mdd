...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<LinkBean> entities = query.fetchInto(LinkBean.class);
		int total = create.selectCount().from(LINKS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toLink(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Link> entityList = entities.stream().map(e -> BeanPOJOConverter.toLink(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Links", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = LINKS.field(split[0]);
			Field<?> field1 = LINKS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(LINKS._ID);
			fields.add(LINKS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = LINKS._ID.equal(Integer.valueOf(idParameter));
		}
