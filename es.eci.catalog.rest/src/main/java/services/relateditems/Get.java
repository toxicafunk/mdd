...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<RelatedItemBean> entities = query.fetchInto(RelatedItemBean.class);
		int total = create.selectCount().from(RELATED_ITEMS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toRelatedItem(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<RelatedItem> entityList = entities.stream().map(e -> BeanPOJOConverter.toRelatedItem(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d RelatedItems", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = RELATED_ITEMS.field(split[0]);
			Field<?> field1 = RELATED_ITEMS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(RELATED_ITEMS._ID);
			fields.add(RELATED_ITEMS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = RELATED_ITEMS._ID.equal(Integer.valueOf(idParameter));
		}
