...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<ProductTypeBean> entities = query.fetchInto(ProductTypeBean.class);
		int total = create.selectCount().from(PRODUCT_TYPES).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toProductType(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<ProductType> entityList = entities.stream().map(e -> BeanPOJOConverter.toProductType(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d ProductTypes", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = PRODUCT_TYPES.field(split[0]);
			Field<?> field1 = PRODUCT_TYPES.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(PRODUCT_TYPES._ID);
			fields.add(PRODUCT_TYPES._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = PRODUCT_TYPES._ID.equal(Integer.valueOf(idParameter));
		}
