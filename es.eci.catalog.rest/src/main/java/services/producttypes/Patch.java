...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		ProductTypesRecord entityRecord = create.fetchOne(PRODUCT_TYPES, PRODUCT_TYPES._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			ProductTypeBean updatedEntity = new ProductTypeBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toProductType(create, updatedEntity), options);
		} else {
