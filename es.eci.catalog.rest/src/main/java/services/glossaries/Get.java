...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<GlossaryBean> entities = query.fetchInto(GlossaryBean.class);
		int total = create.selectCount().from(GLOSSARIES).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toGlossary(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Glossary> entityList = entities.stream().map(e -> BeanPOJOConverter.toGlossary(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Glossaries", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = GLOSSARIES.field(split[0]);
			Field<?> field1 = GLOSSARIES.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(GLOSSARIES._ID);
			fields.add(GLOSSARIES._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = GLOSSARIES._ID.equal(Integer.valueOf(idParameter));
		}
