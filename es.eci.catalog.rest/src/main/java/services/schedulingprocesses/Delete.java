...
import static es.eci.catalog.model.Tables.SCHEDULING_PROCESS_;
...
public class Delete extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		SchedulingProcess_Record record = create.fetchOne(SCHEDULING_PROCESS_, SCHEDULING_PROCESS_._ID_.eq(Integer.valueOf(idParameter)));
		
		int stored = record.delete();
		if (stored == 1) {
			HazelcastInstance hz = getCache();
			IMap<String, List<EntityBean>> cache = hz.getMap("CATALOG_CACHE");
			cache.delete("SchedulingProcess_" + record.get_Id_().toString());
