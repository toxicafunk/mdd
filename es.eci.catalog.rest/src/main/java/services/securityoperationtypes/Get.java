...
public class Get extends ECIServiceVerticle {
	...
	public static SecurityOperationType fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_OPERATION_TYPE_).where(SECURITY_OPERATION_TYPE_._ID_.eq(id)).fetchInto(SecurityOperationType.class).get(0);
	}

	public static List<SecurityOperationType> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(SECURITY_OPERATION_TYPE_).where(SECURITY_OPERATION_TYPE_._ID_.in(ids)).fetchInto(SecurityOperationType.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "SecurityOperationType_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (SecurityOperationType) r.into(SECURITY_OPERATION_TYPE_).into(SecurityOperationType.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d SecurityOperationTypes", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(SECURITY_OPERATION_TYPE_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			logger.info("No securityoperationtypes found!");
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (SecurityOperationType) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (SecurityOperationType) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = SECURITY_OPERATION_TYPE_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(SECURITY_OPERATION_TYPE_._ID_.asc());
			fields.add(SECURITY_OPERATION_TYPE_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(SECURITY_OPERATION_TYPE_);
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = SECURITY_OPERATION_TYPE_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = SECURITY_OPERATION_TYPE_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.security_operation_type_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
