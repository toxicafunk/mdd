...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		try {
			SecurityOperationType_Record record = updateRecord(ctx, Codecs.json2SecurityOperationType(entity));
			message.reply(InverseCodecs.entity2JsonObject(ctx, record.into(new SecurityOperationType()), Collections.<String>emptyList()));
		} catch(Exception e) {
			message.reply(new JsonObject(String.format("{\"errors\": [\"Error al insertar record\", \"%s\"]}", e.getCause())));
		}
	}
	
	public static SecurityOperationType_Record updateRecord(DSLContext ctx, SecurityOperationType entityBean) {
		SecurityOperationType_Record record = ctx.fetchOne(SECURITY_OPERATION_TYPE_, SECURITY_OPERATION_TYPE_._ID_.eq(entityBean.getId()));
		int stored = 0;
		if (record == null) {
		...
		if (stored == 1) {
			...
			cache.replace("SecurityOperationType_" + record.get_Id_().toString(), Arrays.asList(entityBean));
		}
