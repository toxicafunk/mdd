...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<BrandBean> entities = query.fetchInto(BrandBean.class);
		int total = create.selectCount().from(BRANDS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toBrand(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Brand> entityList = entities.stream().map(e -> BeanPOJOConverter.toBrand(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Brands", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = BRANDS.field(split[0]);
			Field<?> field1 = BRANDS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(BRANDS._ID);
			fields.add(BRANDS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = BRANDS._ID.equal(Integer.valueOf(idParameter));
		}
