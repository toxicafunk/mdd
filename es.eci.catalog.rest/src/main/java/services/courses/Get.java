...
public class Get extends ECIServiceVerticle {
	...
	public static Course fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(COURSE_).where(COURSE_._ID_.eq(id)).fetchInto(Course.class).get(0);
	}

	public static List<Course> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(COURSE_).where(COURSE_._ID_.in(ids)).fetchInto(Course.class);
	}

	public static List<Course> fecthByMasterId(DSLContext ctx, Integer id) {
		return ctx.select().from(COURSE_).where(COURSE_._TEACHING_CENTER_ID_.eq(id)).fetchInto(Course.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "Course_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (Course) r.into(COURSE_).into(Course.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d Courses", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(COURSE_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (Course) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (Course) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = COURSE_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(COURSE_._ID_.asc());
			fields.add(COURSE_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(COURSE_);

		if (initialize.contains("materials")) {
		
			clause.join(MATERIAL_, JoinType.LEFT_OUTER_JOIN).on(COURSE_. _ID_.eq(MATERIAL_._COURSE_ID_));
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = COURSE_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = COURSE_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.course_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
