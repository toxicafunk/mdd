...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		Category entity = (Category) message.body();
		
		CategoryBean jooqBean = POJOBeanConverter.entity2Bean(create, entity);
		
		CategoriesRecord entityRecord = create.fetchOne(CATEGORIES, CATEGORIES._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			CategoryBean updatedEntity = new CategoryBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toCategory(create, updatedEntity), options);
		} else {
