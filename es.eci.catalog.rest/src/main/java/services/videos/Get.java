...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<VideoBean> entities = query.fetchInto(VideoBean.class);
		int total = create.selectCount().from(VIDEOS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			options.setCodecName("videosinversecodec");
			headers.add("type", "object");
			message.reply(BeanPOJOConverter.toVideo(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Video> entityList = entities.stream().map(e -> BeanPOJOConverter.toVideo(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Videos", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = VIDEOS.field(split[0]);
			Field<?> field1 = VIDEOS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(VIDEOS._ID);
			fields.add(VIDEOS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = VIDEOS._ID.equal(Integer.valueOf(idParameter));
		}
