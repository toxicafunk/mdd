...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		Video entity = (Video) message.body();
		
		VideoBean jooqBean = POJOBeanConverter.entity2Bean(create, entity);
		
		VideosRecord entityRecord = create.fetchOne(VIDEOS, VIDEOS._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			VideoBean updatedEntity = new VideoBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toVideo(create, updatedEntity), options);
		} else {
