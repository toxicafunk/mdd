...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		try {
			SecurityEntity_Record record = updateRecord(ctx, Codecs.json2SecurityEntity(entity));
			message.reply(InverseCodecs.entity2JsonObject(ctx, record.into(new SecurityEntity()), Collections.<String>emptyList()));
		} catch(Exception e) {
			message.reply(new JsonObject(String.format("{\"errors\": [\"Error al insertar record\", \"%s\"]}", e.getCause())));
		}
	}
	
	public static SecurityEntity_Record updateRecord(DSLContext ctx, SecurityEntity entityBean) {
		SecurityEntity_Record record = ctx.fetchOne(SECURITY_ENTITY_, SECURITY_ENTITY_._ID_.eq(entityBean.getId()));
		int stored = 0;
		if (record == null) {
		...
		if (stored == 1) {
			...
			cache.replace("SecurityEntity_" + record.get_Id_().toString(), Arrays.asList(entityBean));
		}
