...
public class Get extends ECIServiceVerticle {
	...
	public static SecurityAccessPrivilege fecthById(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_ACCESS_PRIVILEGE_).where(SECURITY_ACCESS_PRIVILEGE_._ID_.eq(id)).fetchInto(SecurityAccessPrivilege.class).get(0);
	}

	public static List<SecurityAccessPrivilege> fecthByIds(DSLContext ctx, Integer... ids) {
		return ctx.select().from(SECURITY_ACCESS_PRIVILEGE_).where(SECURITY_ACCESS_PRIVILEGE_._ID_.in(ids)).fetchInto(SecurityAccessPrivilege.class);
	}

	public static List<SecurityAccessPrivilege> fecthByMasterId(DSLContext ctx, Integer id) {
		return ctx.select().from(SECURITY_ACCESS_PRIVILEGE_).where(SECURITY_ACCESS_PRIVILEGE_._SECURITY_ROLE_ID_.eq(id)).fetchInto(SecurityAccessPrivilege.class);
	}
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		String key = ((returnAsList)? queryAsString(query) : "SecurityAccessPrivilege_" + idParameter)  + "_" + initializeParameter;
		...
		int found = 0;
		if (entities == null) {
			entities = query.fetch().stream().map(r -> (SecurityAccessPrivilege) r.into(SECURITY_ACCESS_PRIVILEGE_).into(SecurityAccessPrivilege.class)).collect(Collectors.toList());

			found = entities.size();
			logger.info(String.format("Found %d SecurityAccessPrivileges", found));
			if (found > 0) {
		...
		int total = ctx.selectCount().from(SECURITY_ACCESS_PRIVILEGE_).fetchOne(0, int.class);
		...
		if (entities.isEmpty()) {
			logger.info("No securityaccessprivileges found!");
			...
			List<JsonObject> entityList = entities.stream().map(e -> InverseCodecs.entity2JsonObject(ctx, (SecurityAccessPrivilege) e, initialize)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			message.reply(jsonEntities.encode(), options.setHeaders(headers));
		} else {
			headers.add("type", "object");
			message.reply(InverseCodecs.entity2JsonObject(ctx, (SecurityAccessPrivilege) entities.get(0), initialize), options.setHeaders(headers));
		}
	}
	...
	private List<SortField<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			for (String parameter : splittedParameters) {
				...
				if (split.length == 2) {
					Field<?> field = SECURITY_ACCESS_PRIVILEGE_.field(split[0]+"_");
		...
		if (fields.isEmpty()) {
			fields.add(SECURITY_ACCESS_PRIVILEGE_._ID_.asc());
			fields.add(SECURITY_ACCESS_PRIVILEGE_._LAST_MODIFIED_.asc());
		}
	...
	private SelectJoinStep<Record> fromClause(SelectSelectStep<Record> selectStep, List<String> initialize) {
		SelectJoinStep<Record> clause = selectStep.from(SECURITY_ACCESS_PRIVILEGE_);
	...
	private Condition condition(JsonObject parameters, String idParameter) {
		...
		if (StringUtils.isNotEmpty(idParameter)) {
			...
			if (strIds.length == 1) {
				result = SECURITY_ACCESS_PRIVILEGE_._ID_.equal(Integer.valueOf(strIds[0]));
				...
				result = SECURITY_ACCESS_PRIVILEGE_._ID_.in(ids);
			}
		}

		if (searchParameter != null) {
			result = result.and("public.security_access_privilege_._search_ @@ to_tsquery('spanish', '" + searchParameter + ":*')");
		}
