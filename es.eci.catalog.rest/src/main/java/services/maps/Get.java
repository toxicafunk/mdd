...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<MapBean> entities = query.fetchInto(MapBean.class);
		int total = create.selectCount().from(MAPS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toMap(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Map> entityList = entities.stream().map(e -> BeanPOJOConverter.toMap(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Maps", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = MAPS.field(split[0]);
			Field<?> field1 = MAPS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(MAPS._ID);
			fields.add(MAPS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = MAPS._ID.equal(Integer.valueOf(idParameter));
		}
