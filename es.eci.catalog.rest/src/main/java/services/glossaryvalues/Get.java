...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		SelectConditionStep<Record> query = (SelectConditionStep<Record>) create.select()
				.from(GLOSSARY_VALUES)
				.where(condition(parameters))
				.orderBy(orderFields.get(0), orderFields.get(1))
				.limit(limitParameter + 1).offset(offsetParameter);
				
		List<GlossaryValueBean> entities = query.fetchInto(GlossaryValueBean.class);
		int total = create.selectCount().from(GLOSSARY_VALUES).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toGlossaryValue(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<GlossaryValue> entityList = entities.stream().map(e -> BeanPOJOConverter.toGlossaryValue(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d GlossaryValues", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = GLOSSARY_VALUES.field(split[0]);
			Field<?> field1 = GLOSSARY_VALUES.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(GLOSSARY_VALUES._ID);
			fields.add(GLOSSARY_VALUES._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = GLOSSARY_VALUES._ID.equal(Integer.valueOf(idParameter));
		}
