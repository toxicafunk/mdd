...
import static es.eci.catalog.model.Tables.SECURITY_ROW_LEVEL_POLICY_;
...
public class Delete extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		SecurityRowLevelPolicy_Record record = create.fetchOne(SECURITY_ROW_LEVEL_POLICY_, SECURITY_ROW_LEVEL_POLICY_._ID_.eq(Integer.valueOf(idParameter)));
		
		int stored = record.delete();
		if (stored == 1) {
			HazelcastInstance hz = getCache();
			IMap<String, List<EntityBean>> cache = hz.getMap("CATALOG_CACHE");
			cache.delete("SecurityRowLevelPolicy_" + record.get_Id_().toString());
