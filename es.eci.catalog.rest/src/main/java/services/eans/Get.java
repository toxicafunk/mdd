...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<EanBean> entities = query.fetchInto(EanBean.class);
		int total = create.selectCount().from(EANS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			options.setCodecName("eansinversecodec");
			headers.add("type", "object");
			message.reply(BeanPOJOConverter.toEan(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Ean> entityList = entities.stream().map(e -> BeanPOJOConverter.toEan(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Eans", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = EANS.field(split[0]);
			Field<?> field1 = EANS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(EANS._ID);
			fields.add(EANS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = EANS._ID.equal(Integer.valueOf(idParameter));
		}
