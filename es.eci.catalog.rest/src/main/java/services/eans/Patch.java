...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		Ean entity = (Ean) message.body();
		
		EanBean jooqBean = POJOBeanConverter.entity2Bean(create, entity);
		
		EansRecord entityRecord = create.fetchOne(EANS, EANS._ID.eq(jooqBean.getId()));
		...
		if (numberCreated > 0) {
			...
			EanBean updatedEntity = new EanBean();
			entityRecord.into(updatedEntity);
			updatedEntity.setId(entityRecord.get_Id());
			message.reply(BeanPOJOConverter.toEan(create, updatedEntity), options);
		} else {
