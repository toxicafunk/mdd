...
public class Patch extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext ctx, Message<Object> message)
		...
		try {
			SecurityUser_Record record = updateRecord(ctx, Codecs.json2SecurityUser(entity));
			message.reply(InverseCodecs.entity2JsonObject(ctx, record.into(new SecurityUser()), Collections.<String>emptyList()));
		} catch(Exception e) {
			message.reply(new JsonObject(String.format("{\"errors\": [\"Error al insertar record\", \"%s\"]}", e.getCause())));
		}
	}
	
	public static SecurityUser_Record updateRecord(DSLContext ctx, SecurityUser entityBean) {
		SecurityUser_Record record = ctx.fetchOne(SECURITY_USER_, SECURITY_USER_._ID_.eq(entityBean.getId()));
		int stored = 0;
		if (record == null) {
		...
		if (stored == 1) {
			...
			cache.replace("SecurityUser_" + record.get_Id_().toString(), Arrays.asList(entityBean));
		}
