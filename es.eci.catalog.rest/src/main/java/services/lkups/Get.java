...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<LkupBean> entities = query.fetchInto(LkupBean.class);
		int total = create.selectCount().from(LKUPS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toLkup(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Lkup> entityList = entities.stream().map(e -> BeanPOJOConverter.toLkup(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Lkups", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = LKUPS.field(split[0]);
			Field<?> field1 = LKUPS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(LKUPS._ID);
			fields.add(LKUPS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = LKUPS._ID.equal(Integer.valueOf(idParameter));
		}
