...
public class Get extends ECIServiceVerticle {
	...
	protected void executeService(UserTransaction ut, DSLContext create, Message<Object> message)
		...
		List<SupplierBean> entities = query.fetchInto(SupplierBean.class);
		int total = create.selectCount().from(SUPPLIERS).fetchOne(0, int.class);
		...
		if (entities.size() == 1) {
			...
			message.reply(BeanPOJOConverter.toSupplier(create, entities.get(0)), options.setHeaders(headers));
		} else {
			List<Supplier> entityList = entities.stream().map(e -> BeanPOJOConverter.toSupplier(create, e)).collect(Collectors.toList());
			JsonArray jsonEntities = new JsonArray(entityList);
			headers.add("type", "array");
			logger.info(String.format("Found %d Suppliers", jsonEntities.size()));
	...
	private List<Field<?>> order(JsonObject parameters) {
		...
		if (orderParameter != null) {
			...
			Field<?> field0 = SUPPLIERS.field(split[0]);
			Field<?> field1 = SUPPLIERS.field(split[1]);
		...
		if (fields.isEmpty()) {
			fields.add(SUPPLIERS._ID);
			fields.add(SUPPLIERS._LAST_MODIFIED);
		}
	...
	private Condition condition(JsonObject parameters) {
		...
		if (idParameter != null) {
			result = SUPPLIERS._ID.equal(Integer.valueOf(idParameter));
		}
